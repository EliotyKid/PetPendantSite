...
const ExperienceHome = ({ isLoading }: ExperienceProps) => {
  ...
  useGSAP(() => {
    ...
    gsap.set(coins.map(c => c.position), { y: -10 });
    gsap.set(coins.map(c => c.rotation), { x: Math.PI / 2, y: Math.PI });
    gsap.set(coins.map(c => c.scale), { x: 0.8, y: 0.8, z: 0.8 });

    introTl
      .to(coins.map(c => c.position), { y: 0, stagger: 0.15 })
      .to(coins.map(c => c.rotation), { x: 0, y: 0, stagger: 0.15 }, "<")
      .to(coins.map(c => c.scale), { x: 1, y: 1, z: 1 }, "<");
    ...
    scrollTl.to(centerCoin.position, {
      y: 5,     // Sobe menos agressivamente
      z: -8,    // Vai para o fundo
      ease: "power1.inOut"
    }, 0);

    scrollTl.to(centerCoin.rotation, {
      y: Math.PI, 
      ease: "power1.inOut"
    }, 0);

    // 2. Esquerda (Abre para esquerda)
    scrollTl.to(leftCoin.position, {
    ...
    scrollTl.to(leftCoin.rotation, {
      z: -Math.PI / 3, // Inclina um pouco
      ease: "power1.inOut"
    }, 0);

    // 3. Direita (Abre para direita)
    scrollTl.to(rightCoin.position, {
      x: 6,
      y: 2,
      z: -4,
      ease: "power1.inOut"
    ...
    scrollTl.to(rightCoin.rotation, {
      z: Math.PI / 3,
      ease: "power1.inOut"
    }, 0);
